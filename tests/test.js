...
contract('OrgToken Unit Testing', (accounts) => {
    ...
    // xdescribe("User Functionality", async () => {
    ...
    // xdescribe("Announcement Functionality", async () => {
    ...
    // xdescribe("Item Functionality", async () => {
    //     before(async () => {
    ...
    describe("Bounty Functionality", async () => {
        ...
        it('Users can autonomously award bounties with Twitter Chainlink approval', async () => {
            ...
            let bountyNonce = new BN(7)
            await bounties.applyForBounty(bountyNonce, { from: validTwitterMember1 })
            console.log('Bounty Applied For...')
            let fufilled = false
            while(!fufilled) {
                fufilled = await bounties.checkFufillment(bountyNonce)
                if(fufilled) {
                    expect(await bounties.checkRetweet('388447226', '1321436762816872449')).to.be.true
                    expectEvent.inLogs(
                        logs,
                        'BountyAwarded',
                        { _nonce: bountyNonce, _to: validTwitterMember1, _minted: new BN(100) }
                    )
                } else {
        ...
        it('If a user has not fufilled the bounty the chainlink adapter should return false', async () => {
            ...
            let bountyNonce = new BN(7)
            await bounties.applyForBounty(bountyNonce, { from: validTwitterMember2 })
            console.log('Bounty Applied For...')
            let fufilled = false
            while(!fufilled) {
                fufilled = await bounties.checkFufillment(bountyNonce)
                if(fufilled) {
                    expect(await bounties.checkRetweet('123456', '1321436762816872449')).to.be.false
                    expectEvent.inLogs(
                        logs,
                        'BountyRejected',
                        { _nonce: bountyNonce, _to: validTwitterMember2 }
                    )
                } else {
                    console.log('Awaiting chainlink request fufillment')
                    await sleep(20000)
